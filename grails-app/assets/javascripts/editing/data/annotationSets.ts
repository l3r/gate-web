...
export class AnnotationSets {
    ...
    private annotations : Map<String, Map<String, Array<Annotation>>>;
    private isVisible : Map<String, Map<String, boolean>>;

    constructor() {
        this.lastColourUsed = 0;
        this.typeColours = new Map<String, Map<String, Colour>>();
        this.annotations = new Map<String, Map<String, Array<Annotation>>>();
        ...
        for (var annotationSet in json) {
            for (var annotationType in json[annotationSet]) {
                for (var annotation of json[annotationSet][annotationType]) {
                    result.addAnnotation(annotationSet, annotationType, annotation.startOffset, annotation.endOffset);
                }
            }
        }
     ...
     * @returns {any}
     */
    public getVisibleAnnotations() : Array<Annotation> {
        ...
        this.isVisible.forEach((annotationTypes : Map<string, boolean>, annotationSet) => {
            annotationTypes.forEach((visible : boolean, annotationType: string) => {
                if (visible) {
                    ...
                    this.annotations.get(annotationSet).get(annotationType).forEach((annotation : Annotation) => {
    ...
    public addAnnotation(set : string, type : string, startOffset : number, endOffset:number) : Annotation {
        var annotation = new Annotation(type, set, startOffset, endOffset);

        if (!this.annotations.has(set)) {
            this.annotations.set(set, new Map<string, Array<Annotation>>());
        }

        if (!this.annotations.get(set).has(type)) {
            this.annotations.get(set).set(type, new Array<Annotation>());
        }
